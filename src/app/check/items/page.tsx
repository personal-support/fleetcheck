...
export default function ChecklistItemsPage() {
  ...
  if (!template || !vehicle) {
    return <main className="min-h-screen flex items-center justify-center" style={{ background: '#ebeff2' }}>
      <div className="w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: '#f86924', borderTopColor: 'transparent' }} />
    </main>
  }
    ...
    return (
      <main className="min-h-screen flex flex-col" style={{ background: '#ebeff2' }}>
        ...
        <div className="flex-1 flex flex-col items-center px-5 pt-8 animate-fade-up">
          <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 26, fontWeight: 800, color: '#555555', marginBottom: 4 }}>RESUMO DA SAÍDA</h2>
          <p style={{ color: '#8d949a', fontSize: 13, marginBottom: 20 }}>{vehicle.plate} · {km.toLocaleString('pt-BR')} km</p>
          <div className="w-full max-w-sm">
            {results.map((r, i) => (
              <div key={r.id} className="flex items-center justify-between py-3" style={{ borderBottom: '1px solid #1a2040' }}>
                <span style={{ color: '#555555', fontSize: 14 }}>{items[i]?.icon} {items[i]?.label}</span>
          ...
          <div className="mt-4 w-full max-w-sm px-4 py-3 rounded-xl" style={{ background: 'rgba(33,39,113,0.04)', border: '1px solid rgba(33,39,113,0.08)' }}>
            <p style={{ color: '#f86924', fontSize: 12 }}>🚗 Ao chegar ao destino, abra o app e registre a chegada para fechar esta viagem.</p>
          </div>
          <button onClick={finishChecklist} disabled={saving}
            style={{ marginTop: 20, width: '100%', maxWidth: 360, padding: 14, borderRadius: 10, background: saving ? '#212771' : '#f86924', color: 'white', fontWeight: 700, fontSize: 15, border: 'none', cursor: saving ? 'not-allowed' : 'pointer' }}>
    ...
    <main className="min-h-screen flex flex-col" style={{ background: '#ebeff2' }}>
      <div className="px-5 pt-6 pb-4">
        <div className="flex items-center justify-between mb-3">
          <p style={{ fontSize: 11, color: '#8d949a', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{vehicle.plate} · Saída · Item {currentIndex + 1}/{items.length}</p>
          <span style={{ fontSize: 11, color: '#f86924', fontWeight: 600 }}>{Math.round((currentIndex / items.length) * 100)}%</span>
        </div>
        ...
        {!showNok ? (
          <div className="flex-1 flex flex-col justify-center animate-fade-up">
            <div className="text-center mb-10">
              ...
              <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 36, fontWeight: 800, color: '#555555', marginBottom: 8 }}>{currentItem.label.toUpperCase()}</h2>
              <p style={{ color: '#8d949a', fontSize: 15, lineHeight: 1.5 }}>{currentItem.description}</p>
            </div>
          ...
          <div className="flex-1 flex flex-col animate-fade-up gap-4">
            <div>
              <button onClick={() => setShowNok(false)} style={{ background: 'none', border: 'none', color: '#8d949a', cursor: 'pointer', fontSize: 13, padding: 0, marginBottom: 12 }}>← Voltar</button>
            ...
            {photoField && (
              <div>
                <label style={{ display: 'block', fontSize: 11, color: '#8d949a', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Foto {isPhotoRequired ? '(obrigatória)' : '(opcional)'}</label>
            ...
            <button onClick={submitNok} disabled={isPhotoRequired && !photoBlob}
              style={{ width: '100%', padding: 14, borderRadius: 10, marginTop: 'auto', marginBottom: 20, background: isPhotoRequired && !photoBlob ? '#b6bcc2' : '#ef4444', color: 'white', fontWeight: 700, fontSize: 15, border: 'none', cursor: isPhotoRequired && !photoBlob ? 'not-allowed' : 'pointer' }}>
              REGISTRAR PENDÊNCIA →
            </button>
          </div>
        )}
