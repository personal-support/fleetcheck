...
export default function ChecklistItemsPage() {
  ...
  async function finishChecklist() {
    ...
    const checklistData = { company_id: CONSULDATA_COMPANY_ID, vehicle_id: vehicle.id, user_id: user.id, template_id: template?.id ?? null, status: 'open', departure_km_auto: kmAuto || null, departure_km_final: km || null, departure_km_was_manual: kmWasManual, departure_dt_auto: dtAuto, departure_dt_final: dtFinal, departure_dt_was_manual: false, departure_lat_auto: latAuto, departure_lng_auto: lngAuto, departure_location_lat: latFinal as number | null, departure_location_lng: lngFinal as number | null, departure_location_was_manual: locationWasManual, departure_items: results.map(r => { const t = template?.items.find(i => i.id === r.id); return { ...r, label: t?.label ?? r.id, icon: t?.icon ?? '' } }) as ChecklistItemResult[], departure_notes: null as string | null, created_at: new Date().toISOString() }
    if (navigator.onLine) {
      try {
